...
export default function SearchPage() {
  ...
  useEffect(() => {
    const loadTours = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Khong tai duoc du lieu tour.");
        }
        ...
        setReloadToast("error", "Khong tai duoc du lieu tour.");
  ...
  const filteredTours = useMemo(() => {
    ...
    return nextTours;
  }, [tours, query, selectedCategories, minPrice, maxPrice, departureFrom, sort]);
  ...
  return (
    <main className="pt-16 pb-32">
      <TourHero query={query} onQueryChange={setQuery} title="Where to next?" subtitle="Explore curated tours with flexible filters and realtime availability." />
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <section className="grow">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-extrabold tracking-tight">{filteredTours.length} ket qua</h2>
            ...
            <TourList tours={filteredTours} loading={loading} emptyMessage="Khong co ket qua phu hop." />
          </section>
        </div>
      </div>
    </main>
