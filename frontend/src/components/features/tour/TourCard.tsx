...
export function TourCard({ tour }: TourCardProps) {
  ...
  return (
    <article className="group bg-surface-container-lowest rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-[0_24px_48px_-12px_rgba(17,24,39,0.12)] border border-surface-container-high/50">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            href={`/tour/${tour.slug}`}
            className="bg-[#5EEB5B] text-[#004708] w-10 h-10 rounded-full text-sm font-extrabold hover:opacity-80 transition-opacity inline-flex items-center justify-center"
          >
            <FaArrowRight />
          </Link>
        </div>
